...
<task type="auto">
  ...
  <files>apps/web/app/routes/search.tsx, apps/web/app/components/search/follow-up.tsx, apps/web/app/routes/api.search.tsx, apps/web/app/services/rag.server.ts</files>
  <action>
...
**Update `saveSearchResultCache` in `api.search.tsx`:**
In Plan 02, `saveSearchResultCache` was created with `suggested_followups: []` hardcoded. Update this call to pass the actual generated followups array instead of the empty array:
```typescript
// Replace: suggested_followups: []
// With: suggested_followups: followups
saveSearchResultCache({ query, answer, sources, suggested_followups: followups, source_count: sources.length })
```
This ensures cached results include the follow-up suggestions, so when a user loads a shared URL via `?id=...`, the follow-up chips still appear.
