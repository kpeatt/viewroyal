...
<task type="auto">
  ...
  <action>
Create `apps/web/app/components/profile/` directory with 4 components:
...
- Time range selector (if implemented): tabs for "Last 12 months", "Current term", "All time". Pass as prop to parent for re-fetching. Default to all time for simplicity (can add date filtering later).
...
**stance-spectrum.tsx** -- `StanceSpectrum` component:
...
- Width: full width of parent card
  ...
  <done>All 4 components created, typed, and compiling. SpeakingTimeCard shows headline + sparkline + topic bars. SpeakerRanking shows horizontal bar chart. StanceSummary renders per-topic stance cards with evidence. StanceSpectrum renders position indicator.</done>
</task>
...
2. Add these to the Promise.all in the loader (they're independent of each other):
   - `getSpeakingTimeStats(supabase)` -- for ranking all councillors
   - `getSpeakingTimeByMeeting(supabase, id)` -- for trend chart
   - `getSpeakingTimeByTopic(supabase, id)` -- for topic breakdown
   - `getCouncillorStances(supabase, id)` -- for stance summaries
3. Return new data in loader response: `speakingTimeRanking`, `speakingTimeTrend`, `speakingTimeByTopic`, `stances`
4. Compute `currentPersonSpeakingTime` from the ranking data (find by person_id) for the headline stat
