...
9. **Create unique indexes** per municipality (agenda_items uses meeting_id scope since it lacks municipality_id):
   ```sql
...
3. No duplicate slugs per municipality in any table (agenda_items globally unique)
