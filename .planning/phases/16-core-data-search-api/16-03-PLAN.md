...
- `serializeBylawDetail(row, related)` -- Returns: `{ ...summary fields, description, plain_english_summary, matters (array of { slug, title, status } matter summaries linked via bylaw_matters or agenda items) }`.
...
- Lookup by slug + municipality_id. Fetch related matters (via agenda_items that reference both the bylaw and matters, or via a direct bylaw_matters join if table exists -- check DB schema). Return matter summaries inline.
