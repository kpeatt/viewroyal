...
must_haves:
  truths:
    ...
    - "update_user_location RPC exists (SECURITY DEFINER) for safe geography column writes from the web app"
...
**4. Create `update_user_location` RPC** for safely updating the geography column from the web app:
```sql
...
$$;
```
This uses SECURITY DEFINER so RLS on the geography column update is bypassed. The web app calls it via `supabase.rpc('update_user_location', { target_user_id, lng, lat })`.

**5. Update `find_meeting_subscribers` RPC** to add two new UNION branches:
...
```
  ...
  <verify>Run verification queries via `mcp__supabase__execute_sql` to confirm: topics has 8 rows, subscriptions has keyword + keyword_embedding columns, user_profiles has onboarding_completed column, find_meeting_subscribers function exists with topic matching branches, update_user_location RPC exists.</verify>
  <done>Topics table seeded with 8 categories. Subscriptions table has keyword and keyword_embedding columns with HNSW index. user_profiles has onboarding_completed boolean. find_meeting_subscribers RPC matches topic subscriptions by both category FK and keyword embedding similarity. update_user_location RPC exists for safe geography column updates from the web app.</done>
</task>
