...
- If address provided and geocoded: call `upsertUserProfile` with address and update `location` via raw SQL: `UPDATE user_profiles SET location = ST_SetSRID(ST_MakePoint(lng, lat), 4326)::geography WHERE id = userId`
...
- The action handler should run `UPDATE user_profiles SET location = ST_SetSRID(ST_MakePoint(lng, lat), 4326)::geography WHERE id = userId` when lat/lng are provided
...
- When lat/lng are provided, use `supabase.rpc` or raw SQL to update the geography column
