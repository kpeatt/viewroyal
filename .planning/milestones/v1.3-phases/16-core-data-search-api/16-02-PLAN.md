...
- Handler: The `people` table does NOT have a `municipality_id` column. To scope people to the current municipality, join through memberships -> organizations and filter by `organizations.municipality_id`. Use: `supabase.from("people").select("..., memberships!inner(organizations!inner(municipality_id))").eq("memberships.organizations.municipality_id", muni.id)`. Apply optional filters: for `is_councillor`, filter on membership role/active status. For `name`, use `.ilike("name", `%${name}%`)`. Sort by `name ASC, id ASC`. Apply cursor pagination. Map through `serializePersonSummary`. Return via `listResponse`.

**people/detail.ts -- GetPerson:**
- Schema request.params: `slug`, `municipality`.
- Handler: The `people` table does NOT have a `municipality_id` column. Look up person by slug alone using `.eq("slug", slug).maybeSingle()`. Then verify the person belongs to the municipality by checking their memberships join to organizations in that municipality (or simply accept slug-only lookup since people names are unique in the current single-municipality dataset). If not found, throw `ApiError(404, "PERSON_NOT_FOUND", ...)`. Fetch memberships and vote counts in parallel. Serialize via `serializePersonDetail`. Return via `detailResponse`.
