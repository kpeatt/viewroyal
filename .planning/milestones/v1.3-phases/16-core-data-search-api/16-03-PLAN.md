...
- The `motions` table does NOT have a `municipality_id` column. Scope motions to the municipality via an inner join on meetings: `supabase.from("motions").select("..., meetings!inner(slug, meeting_date, municipality_id)").eq("meetings.municipality_id", muni.id)`. This ensures only motions from meetings belonging to the current municipality are returned.
- Also join with `people` (mover/seconder via mover_id, seconder_id) and `agenda_items` (for agenda_item reference).
- For `meeting` filter: filter by `meetings.slug` (already joined).
- For `mover` filter: join people via mover_id and filter by `people.slug`.

**motions/detail.ts -- GetMotion:**
- The `motions` table does NOT have a `municipality_id` column. Lookup by slug alone, then verify municipality ownership via the joined meeting: `supabase.from("motions").select("..., meetings!inner(slug, meeting_date, municipality_id)").eq("slug", slug).eq("meetings.municipality_id", muni.id).maybeSingle()`. This returns null if the motion doesn't belong to the municipality. Fetch motion with person joins (mover, seconder), meeting join, agenda_item join. Fetch votes with person join in parallel. Include full roll call in `votes` array.
