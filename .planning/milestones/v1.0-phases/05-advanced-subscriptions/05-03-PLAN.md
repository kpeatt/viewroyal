...
1. The `find_meeting_subscribers` RPC already does ALL the matching work — it returns rows with `(user_id, subscription_id, subscription_type, notification_email)`. Each row represents a specific subscription that matched for the given meeting. A single subscriber may appear in multiple rows if multiple subscriptions matched.

2. **Highlighting strategy — use the RPC results, do NOT re-do matching logic:**
   - Group the RPC results by `notification_email` to get each subscriber's matched subscription IDs and types.
   - For each subscriber, query their full subscription details (target_id, keyword, topic_id, etc.) for the matched `subscription_id` values.
   - Then, for each agenda item/decision in the digest, determine highlighting:
     - `matter` subscriptions: highlight decisions where the motion's `matter_id` matches the subscription's `target_id`
     - `person` subscriptions: highlight decisions involving the subscribed person's `target_id` (match against attendance or motion mover/seconder)
     - `topic` subscriptions (category): highlight items where the matter's `category` matches the subscribed topic's name (join via `topic_id` -> `topics.name`)
     - `topic` subscriptions (keyword): highlight items whose embeddings had cosine similarity match (the RPC already confirmed the match — highlight ALL agenda items with embeddings for keyword subscribers, since the RPC matched at the meeting level)
     - `neighborhood` subscriptions: the RPC already confirmed a geo match exists in this meeting — highlight agenda items that have `geo` data (i.e., a geocoded `related_address`), since those are the items the spatial query matched against
   - The key insight: the RPC has already confirmed WHICH subscription types matched for WHICH subscribers. The Edge Function only needs to map those matched subscription types back to specific items for visual highlighting.
...
6. **Pass matched subscriptions into buildDigestHtml** — for each subscriber email, pass their matched subscription details (from the grouped RPC results). Create a helper function `getHighlightedItems(digest, subscriberMatchedSubs)` that:
   - Accepts the digest data and this specific subscriber's matched subscription details (subscription_id, subscription_type, target_id, keyword, topic_id)
   - Returns a Map of agenda_item_id -> reason string (e.g., "Development (topic)", "Housing keyword", "Near your address", "Matter: 123 Island Hwy rezoning")
   - For matter/person/category-topic subscriptions: match by target_id or topic name against the digest items
   - For keyword-topic subscriptions: mark all agenda items with embeddings as highlighted (RPC already confirmed match)
   - For neighbourhood subscriptions: mark agenda items with `geo` data as highlighted (RPC already confirmed spatial match)

**Note:** The buildDigestHtml function currently builds a generic digest for all subscribers. Since the email is built per subscriber (the loop already iterates per email), the highlighting is subscriber-specific. Query each subscriber's matched subscription details once before the HTML build loop.
  </action>
