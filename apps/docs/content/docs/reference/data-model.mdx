...
<Callout type="info">
  Not all database columns are exposed through the API. The API returns curated fields designed for developer consumption. See the [API Reference](/api-reference) for complete endpoint documentation with request and response schemas.
</Callout>
