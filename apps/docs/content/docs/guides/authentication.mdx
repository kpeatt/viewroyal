...
All data endpoints on the ViewRoyal.ai API require an API key for authentication. The API is read-only -- there are no write, update, or delete operations. The only unauthenticated endpoint is the [health check](/docs/api-reference/system/get_HealthEndpoint).
...
For the complete list of all error codes across the API, see the [Error Handling guide](/docs/guides/error-handling).
